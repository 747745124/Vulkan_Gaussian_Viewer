...
namespace Utils
{
    ...
    inline VkImageView createImageView(const VkDevice &device, const VkImage &image, VkFormat format, VkImageAspectFlags aspectFlags)
    {
    ...
    inline VkFormat findSupportedFormat(const VkPhysicalDevice &device, const std::vector<VkFormat> &candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
    {
        // check if the format is supported by the device
        // pick the best format possible
    ...
    inline VkFormat findDepthFormat(const VkPhysicalDevice &device)
    {
